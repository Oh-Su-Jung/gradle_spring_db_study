...
public class MainForCPS {
	...
	public static void main(String[] args) {
		try(AnnotationConfigApplicationContext ctx = new AnnotationConfigApplicationContext(AppCtx.class)){
			...
			memberDao.insert(new Member("test1@test.co.kr", "1234", "test", LocalDateTime.now()));
			System.out.println("회원은 추가했습니다.");
			
			ChangePasswordService cps = ctx.getBean(ChangePasswordService.class);
			cps.changePassword("test1@test.co.kr", "1234", "new1234");
			System.out.println("암호를 변경했습니다.");
			
			Member member = memberDao.selectByEmail("test1@test.co.kr");
